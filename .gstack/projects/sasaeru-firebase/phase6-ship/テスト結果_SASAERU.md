...
# バグ修正・データクリーンアップ テスト結果レポート（追補 v1.3）

**実施日**: 2026-04-16  
**実施者**: G-Stack AI Testerエージェント  
**対象バージョン**: SASAERU v1.3（バグ修正3件 + 仮データ削除）  
**テスト方法**: コードレビューベース静的テスト
...
| テストID | 項目 | 旧結果 | 新結果 | 修正内容 |
|---------|------|--------|--------|---------|
| M-01b | mypage.html ページ遷移後リダイレクト | ❌ BUG | ✅ PASS | `onAuthStateChanged` の race condition を Promise ベース（一発確定）に変更。ページ遷移直後の `null` 誤判定によるトップページへの強制遷移を解消。 |
| A-01b | admin.html ログイン無限ループ | ❌ BUG | ✅ PASS | Custom Claims 未設定時に `auth.signOut()` → `onAuthStateChanged` 再発火 → ループする問題を修正。サインアウトを行わず `showLogin()` + エラーメッセージ表示のみに変更。`getIdTokenResult(true)` で強制リフレッシュも追加。admin.html も Promise ベース認証に統一。 |
| R-02b | Firestore menus list ルール | ❌ BUG | ✅ PASS | `allow read` を `allow get` / `allow list` に分離。`list` 操作時は `resource` が null になるため `resource.data.status` チェックを削除し、`allow list: if isOwnerOf(orgId)` のみに変更。mypage.html でのメニュー一覧取得エラーを解消。 |

---

## 12. データクリーンアップテスト（v1.3）

| テストID | テスト項目 | 結果 | 詳細 |
|---------|-----------|------|------|
| D-01 | SAMPLE_ORGS 削除（index.html） | ✅ PASS | `index.html` の `SAMPLE_ORGS` 配列（仮クラブ3件）を完全削除。`loadStats()` の `orgCount` 初期値を `SAMPLE_ORGS.length` → `0` に変更。`loadOrgs()` のフォールバックを `allOrgs = SAMPLE_ORGS` → `allOrgs = []` に変更。Firestore未接続時は空リスト + 「該当するクラブはありません」メッセージを表示。 |
| D-02 | DEMO_ORG/DEMO_MENUS/DEMO_POSTS 削除（mypage.html） | ✅ PASS | `mypage.html` のデモデータ3定数（`DEMO_ORG`, `DEMO_MENUS`, `DEMO_POSTS`）を完全削除。Firebase未設定時のフォールバックを「デモデータ表示」から「no-org-screen 表示」に変更。本物のデータのみ扱うクリーンな状態になった。 |

**データクリーンアップテスト合計: 2/2 PASS**
...
| ID | 項目 | 対応 |
|----|------|------|
| M-01b | mypage.html ログイン後リダイレクト | ✅ Promise ベース認証で race condition 解消 |
| A-01b | admin.html 管理者ログイン無限ループ | ✅ signOut ループを排除、Promise ベース認証に統一 |
| R-02b | Firestore menus list ルール | ✅ `allow get` / `allow list` を分離してエラー解消 |
...
| カテゴリ | PASS | WARN | FAIL | 合計 |
|---------|------|------|------|------|
| バグ修正確認テスト (v1.3) | 3 | 0 | 0 | 3 |
| データクリーンアップテスト (v1.3) | 2 | 0 | 0 | 2 |
| **v1.3追加分 合計** | **5** | **0** | **0** | **5** |

---

## 全バージョン累計テスト結果（v1.3時点）
...
※ S-02/M-08/C-09 は v1.2 で、M-01b/A-01b/R-02b は v1.3 で修正済み → PASS移行
...
**v1.3 での改善点:**
- ログイン後リダイレクト問題（mypage.html race condition）を根本修正
- 管理者ログイン無限ループを解消。管理者設定手順（`setCustomUserClaims`）を実行すれば即座にログイン可能
- Firestore menus サブコレクションの `list` ルールを修正し、マイページのメニュー一覧エラーを解消
- 仮クラブデータ・デモデータを完全削除。本番環境でのクリーンな初期状態を保証

**残存課題:**
- U-04（アクセシビリティ）: フォームのlabel/id関連付けは非機能的な改善項目であり、リリースのブロッカーではない

**管理者アカウント設定（必須手順）:**  
`admin: true` Custom Claims は Firebase Admin SDK 経由での付与が必要。  
Node.js スクリプト例: `admin.auth().setCustomUserClaims(uid, { admin: true })`

**総合評価: 全79テスト中78 PASS、1 WARN（アクセシビリティのみ）、FAIL 0件**

---

*追補v1.3レポート作成: G-Stack AI Testerエージェント / 2026-04-16*
